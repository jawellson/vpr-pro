...
class ResNet(nn.Module):
    def __init__(self, input_size, layers=[3, 4, 6, 3], num_filters=[32, 64, 128, 256], embd_dim=192,
        ...
        self.layer1 = self._make_layer(Bottleneck, num_filters[0], layers[0])
        self.layer2 = self._make_layer(Bottleneck, num_filters[1], layers[1], stride=(2, 2))
        self.layer3 = self._make_layer(Bottleneck, num_filters[2], layers[2], stride=(2, 2))
        self.layer4 = self._make_layer(Bottleneck, num_filters[3], layers[3], stride=(2, 2))

        cat_channels = num_filters[3] * Bottleneck.expansion * (input_size // 8)
        if pooling_type == "ASP":
